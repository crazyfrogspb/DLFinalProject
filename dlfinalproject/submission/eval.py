...
def load_data(data_dir, batch_size, split):
    ...
    data = datasets.ImageFolder(
        f'{data_dir}/supervised/{split}', transform=transform)
...
def evaluate(model, data_loader, device, split, top_k=5):
    ...
    for img, target in data_loader:
        ...
        n_correct_top_1 += pred_top_1.eq(target.view_as(pred_top_1)
                                         ).int().sum().item()
...
if __name__ == '__main__':
    ...
    # DO NOT modify this line - if your Model() takes arguments, they should have default values
